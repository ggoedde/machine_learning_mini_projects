...
class SkipGramModel:
    ...
    def _create_summaries(self):
        """Create summaries that can be viewed in TensorBoard."""
...
def train_model(model, batch_gen, num_train_steps, weights_fld):
    ...
    with tf.Session() as sess:
        ...
        # define total loss, used calculate late average loss in the last SKIP_STEP steps
        total_loss = 0.0
        writer = tf.summary.FileWriter('./tb_graph/lr' + str(LEARNING_RATE), sess.graph)
        initial_step = model.global_step.eval()
        for index in range(initial_step, initial_step + num_train_steps):
            # get batch of center words and contexts (targets)
